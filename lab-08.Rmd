...
```{r}
weather_noNA <- weather %>% na.omit(RainTomorrow)
...
```{r}
city_weather <- weather %>% select(
  Date, Location, RainToday) %>% filter(RainToday == "Yes") %>%
  group_by(Location) %>%
  count(RainToday) %>%
  arrange(desc(n))
...
```{r}
RainTommorow_fit <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(RainTomorrow ~ RainToday, data = weather_Portland, 
      family = "binomial")

predict(RainTommorow_fit, weather_Portland, type = "prob") %>%
  bind_cols(weather_Portland %>% select(RainToday, Date))
```
